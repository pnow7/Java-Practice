package study.data_structure.que;
...
public class DequeArrayDeqque {
	...
	public static void main(String[] args) throws IOException {
      ...
      /*
       * new int[]{i, Integer.parseInt(st.nextToken())} 쓰는 이유
       * Deque<Integer> indexDeque = new ArrayDeque<>();
       * Deque<Integer> moveDeque = new ArrayDeque<>();
       * 2개씩 써야하므로 헷갈릴 수 있다
       * i번 풍선, 이동할 값      
       * i는 배열에 넣고       
       * Integer.parseInt(st.nextToken())은 deque에 넣음
       */
      ...
      while (!deque.isEmpty()) {
      	  // balloon은 int[2]의 형태
      	  // new int[]{i, Integer.parseInt(st.nextToken())} 이런 형태니까
      	  // 배열값 i는 index로 들어가고
    	  // 입력값은 move로 들어감
          ...
          // 이동할 방향 결정
          // 오른쪽으로 이동, 이미 하나 뽑았으므로 move-1만큼 이동
          if (move > 0) { 
